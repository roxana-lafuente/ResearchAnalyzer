        ...
        for index, info in enumerate(array_to_parse):
            ...
            if ((
                ...
                if process_name not in self.group_names:
                    ...
                    stylized_group_names[
                        process_name] = "" if process_name == "|" else process_name
                if clicks:
                    neccesary_information = self.parse_click_info(
                        info, stylized_click_names)
                else:
        ...
        if clicks:
            ...
            for clustering_option in self.clustering_options:
                ...
                if neccesary_information_array:
                    self.injector.prepareForHTML(json.dumps(
                        neccesary_information_array), stylized_group_names.values(), "clicks", vis_index)
        else:
            vis_index = self.clustering_options.index(clustering_option)
            if neccesary_information_array:
                self.injector.prepareForHTML(json.dumps(
                    neccesary_information_array), stylized_group_names.values(), "sentences", vis_index)
