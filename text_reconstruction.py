...
class TextReconstructor:
    ...
    def replay_key_function(self, key, time = 0, msg = "key_down", delay=0):
        ...
        if(len(key) == 1):
            ...
            self.append_to_screen(self.define_up_down_behaviour(msg, "", key), time)
        else:
            # It's a function key.
    ...
    def replay_backspace(self, msg, time = 0):
        if msg == KDOWN:
            ...
            self.keys_and_time = self.keys_and_time[0:self.ycursor-1]+self.keys_and_time[self.ycursor:len(self.keys_and_time)]
            self.ycursor -= 1
